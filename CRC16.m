function [high, low] = CRC16(data)


%Convert hex tables from source code to decimal
%C code: https://github.com/ArduPilot/SiK/blob/master/Firmware/radio/crc.c
...
tab_new1 = strrep(tab1,'0x','');
split1 = split(tab_new1,',');
Values1 = hex2dec(split1);
B1 = reshape(Values1,[16, 16]);
A1 = transpose(B1);
table1 = reshape(A1,[1, 256]);
...
tab_new2 = strrep(tab2,'0x','');
split2 = split(tab_new2,',');
Values2 = hex2dec(split2);
B2 = reshape(Values2,[16, 16]);
A2 = transpose(B2);
table2 = reshape(A2,[1, 256]);
...
for i = 1:length(data)
    ...
    k = bitshift(high, 1,'uint8');
    
    if(high > 127)
        high = bitxor(low, table2(k+1),'uint8');
        low = bitxor(b, table2(k+2),'uint8');
    else
        high = bitxor(low, table1(k+1),'uint8');
        low = bitxor(b, table1(k+2),'uint8');
    end
